...
public class AIVehicleScript : MonoBehaviour
{
    ...
    private NodeScript nodeComponenet;
    ...
    void Start()
    {
        // player = AIController.manager.player;
        currentFwdSpeed = forwardSpeed;

        if (vehicleStatus == VehicleStatus.AI)
        {
            AIActive = true;
            if (currentNode && currentNode.GetComponent<NodeScript>().mode == "OnWay") oneWay = true;
        }
        else
        {
            AIActive = false;
    ...
    void Update()
    {
        ...
        {
            if (nextNode.GetComponent<NodeScript>())
            {
                ...
                {
                    nextNode = nextNode.GetComponent<NodeScript>().previousNode;
                }
            }
        }
        if (nextNode.GetComponent<WaysControlScript>())
        {
            if (wayMove == WayMove.Right)
            {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (Physics.Raycast(raycastPoint.TransformPoint((Mathf.Repeat((float)i, 3) - 1), 0, 0), raycastPoint.TransformDirection((rotate * rayCastAngle), 0, (lentgh * rayCastLentgh)), out raycastHit, (lentgh * rayCastLentgh), layerMask.value))
            {
                ...
                if (raycastHit.transform.root.GetComponent<AIVehicleScript>() && raycastHit.transform.root.GetComponent<AIVehicleScript>().trafficStop == true &&
                   Mathf.Abs(Quaternion.Dot(transform.rotation, raycastHit.transform.root.rotation)) > 0.5f)
                    AIBrake = raycastHit.transform.root.GetComponent<AIVehicleScript>().AIBrake;
    ...
    public void widthDistanceRefrash(NodeScript node)
    {
    ...
    void ExtraAI()
    {
        if (currentNode != null)
        {
            NodeScript node = currentNode.gameObject.GetComponent<NodeScript>();
            if (node != null)
            {
        ...
        if (nextNode.GetComponent<WaysControlScript>() && !isTurning)
        {
            isTurning = true;
        ...
        if (isTurning)
        {
            ...
            {
                isTurning = currentNode.GetComponent<NodeScript>();
            } else
            {
    ...
    void AIControl()
    {
        ...
        if (currentNode != null)
        {
            ...
            if (currentNode.GetComponent<WaysControlScript>())
            {
                if (!waysActive)
                {
                    var waysScript = currentNode.GetComponent<WaysControlScript>();
            ...
            {
                nodeComponenet = currentNode.GetComponent<NodeScript>();
                widthDistanceRefrash(currentNode.GetComponent<NodeScript>());
    ...
    Transform RandomWay(Transform inter, Transform prev)
    {
        ...
        WaysControlScript waysControl = inter.GetComponent<WaysControlScript>();
                ...
                switch (randomWays)
                {
                    case 1:
                        ...
                        if (waysControl.way1 != prev)
                        {
                            ...
                            myStatue = waysControl.way1.GetComponent<NodeScript>().nodeState;
                    ...
                    case 2:
                        ...
                        if (waysControl.way2 != prev)
                        {
                            ...
                            myStatue = waysControl.way2.GetComponent<NodeScript>().nodeState;
                    ...
                    case 3:
                        if (waysControl.way3 != prev)
                        {
                            ...
                            myStatue = waysControl.way3.GetComponent<NodeScript>().nodeState;
                    ...
                    case 4:
                        if (waysControl.way4 != prev)
                        {
                            ...
                            myStatue = waysControl.way4.GetComponent<NodeScript>().nodeState;
        ...
        if (lastNode != null)
            widthDistanceRefrash(lastNode.GetComponent<NodeScript>());
    ...
    void OnDrawGizmos()
    {
        ...
        if (raycastPoint)
            Gizmos.color = raycastPoint.GetComponent<GizmoObjectScript>().gizmoColor;
        else
