...
public enum TrafficMode {Go=1,Wait=2,Stop=3};
public class NodeScript : MonoBehaviour
{
    public Transform previousNode;
    public Transform nextNode;

    public float widthDistance = 2.0f; // width distance (Street)
    ...
    void Awake()
    {
        ...
        if (nextNode)
            if (nextNode.GetComponent<WaysControlScript>()) nodeState = "NextPoint"; else nodeState = "PreviousPoint";
        else
